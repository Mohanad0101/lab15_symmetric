...
- `images/` — иллюстрации к темам ноутбука (см. раздел **«Иллюстрации»** ниже).

---

## Теоретический контекст: триада CIA и виды шифрования

Ниже — краткая справка, связывающая лабораторную с общей моделью информационной безопасности. **Триада CIA** задаёт три базовые цели защиты. **Симметричное и асимметричное шифрование** — разные способы обеспечить в первую очередь **конфиденциальность**; **целостность** и **аутентичность** данных в продакшене часто добавляют отдельными механизмами (MAC, подписи, AEAD).
...
### Связь с этой лабораторной

В работе реализуется только **симметричное** шифрование в учебных моделях. Триада **CIA** помогает разделять цели: **конфиденциальность** (шифрование), **целостность/аутентичность** (другие механизмы), **доступность** (инфраструктура). Асимметричные протоколы в ноутбуке **не программируются**, но контекст «симметрия / асимметрия» полезен для общего курса.
...
2. `caesar_crack_assuming_space`
3. функции Vernam/XOR (повторяющийся ключ)
4. функции OTP
5. CBC-подобные функции
6. TCP-интеграция (server/client)

Практические рекомендации:
- проверяйте каждый блок сразу после реализации;
- тестируйте граничные случаи (пустая строка, отрицательный ключ, короткий и длинный текст);
- на шифровании и дешифровании используйте одну и ту же конфигурацию (алгоритм и ключ).

---

## 6) Обязательная TCP-часть: обмен зашифрованными сообщениями
...
- одинаковый алгоритм и ключ на обеих сторонах;
- явные границы сообщений (например, построчный протокол с `\n`);
- корректная обработка закрытия соединения и ошибок декодирования;
- предсказуемый и однозначный формат транспортных данных.
...
- краткий отчёт:
  ...
  - краткое сравнение Цезаря и XOR/CBC.
